...
import lesson_13.student.exceptions as student_exceptions
...
class NameValidate:
    ...
    def __set__(self, instance, value):
        if not self.title_check(value) or not self.char_check(value):
            raise student_exceptions.InvalidNameException(self.param_name, value)
...
class Student:
    ...
    _first_name = NameValidate(str.istitle, str.isalpha)
    _last_name = NameValidate(str.istitle, str.isalpha)
    ...
    def _check_values(self, subject: str, value: int, min_value: int, max_value: int):
        if subject not in self.__subjects_results:
            raise student_exceptions.InvalidSubjectException(subject, self.full_name)
        if value > max_value or value < min_value:
            raise student_exceptions.InvalidGradeException(value, min_value, max_value)
...
if __name__ == '__main__':
    ...
    # print(s1.add_grade('Математика', 0))
    # print(s1.add_test_rate('Математика', 105))
    # print(s1.add_test_rate('Ааа', 100))
